...
export default function Home() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Add to history if original
      if (!data.is_duplicate) {
        setHistory((prev) => [
          { id: Date.now(), hash: data.hash, owner_name: data.original_owner, registered_at: data.registered_at },
          ...prev,
        ]);
      }
    } catch (err) {
          ...
          <div style={{ opacity: mounted ? 1 : 0, transform: mounted ? "translateY(0)" : "translateY(20px)", transition: "all 0.6s ease" }}>
            ...
            <div className="text-center mb-10">
              ...
              <p className="text-sm" style={{ color: "rgba(255,255,255,0.35)" }}>
                Upload your creation — we'll generate a visual fingerprint and register ownership instantly
              </p>
            </div>
            ...
            <div className="rounded-2xl p-6 mb-6"
              ...
              <button onClick={handleUpload} disabled={loading}
                ...
                style={{
                  background: loading ? "rgba(0,255,180,0.05)" : "linear-gradient(135deg, rgba(0,255,180,0.15), rgba(120,80,255,0.15))",
                  border: "1px solid rgba(0,255,180,0.3)",
                  color: loading ? "rgba(255,255,255,0.3)" : "#00ffb4",
                  boxShadow: loading ? "none" : "0 0 20px rgba(0,255,180,0.08)",
                }}
                onMouseEnter={(e) => { if (!loading) e.currentTarget.style.boxShadow = "0 0 30px rgba(0,255,180,0.15)"; }}
                onMouseLeave={(e) => { e.currentTarget.style.boxShadow = loading ? "none" : "0 0 20px rgba(0,255,180,0.08)"; }}>
                {loading ? (
            ...
            {history.length === 0 ? (
              <div className="text-center py-20 rounded-2xl"
                ...
                <button onClick={() => setActiveTab("upload")}
                  className="mt-6 px-6 py-2 rounded-xl text-sm font-semibold transition-all"
              ...
              <div className="flex flex-col gap-3">
                {history.map((art, i) => (
                  <div key={art.id} className="rounded-xl p-4 flex items-center gap-4"
